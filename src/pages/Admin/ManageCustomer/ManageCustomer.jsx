...
const ManageCustomerPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await nikaApi.getAllUsers();
        console.log(response.data);
        const filtedRoleData = response.data.filter((row) => row.role === "CUSTOMER");
  ...
  const handleYes = async () => {
    ...
    else 
    {
      ...
      const subFormState = {...formState, role: "CUSTOMER"};
  ...
  const handleCreateApi = async (subFormState) => {
    try {
      const submit = {
        firstname: subFormState.firstname,
        lastname: subFormState.lastname,
        role : "CUSTOMER",
        password : "Newuser123",
        email: subFormState.email,
      }

      const response = await nikaApi.createCustomer(submit);
  ...
  const handleUpdateApi = async (subFormState) => {
    try{
      const submit = {
        firstName: subFormState.firstname,
        lastName: subFormState.lastname,
        phone: subFormState.phone,
        address: subFormState.address,
        }
      console.log(submit);
      const response = await nikaApi.updateUser(subFormState.id, submit);
  ...
  const handleDeleteApi = async () => {
    try {
      await nikaApi.deleteUser(deletingId);
